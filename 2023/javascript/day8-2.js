...
function getGreatestCommonDivisoderOf(a, b) {
    if (b === 0) {
        return a;
    }
...
function getLeastCommonMultipleOf(numbers) {
    if (numbers.length === 0) {
        return 1;
    }
    let result = numbers[0];
    for (let i = 1; i < numbers.length; i++) {
        result = (result * numbers[i]) / getGreatestCommonDivisoderOf(result, numbers[i]);
    }
    return result;
}
...
export function run(input) {
    ...
    let destinations = Object.keys(nodes).filter(function (node) {
        return node[2] === "A"
    }).map((destination) => {
        ...
        while (true) {
            ...
            next = nodes[next][instructions[currentStep]]
            totalSteps++;
            currentStep++
